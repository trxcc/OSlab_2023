...
void printf(const char *format,...){
	int i __attribute__((unused))=0; // format index
	char buffer[MAX_BUFFER_SIZE] __attribute__((unused));
	int count __attribute__((unused))=0; // buffer index
	int index __attribute__((unused))=0; // parameter index
	void *paraList __attribute__((unused))=(void*)&format + sizeof(uint32_t); // address of format in stack
	int state __attribute__((unused))=0; // 0: legal character; 1: '%'; 2: illegal format
	int decimal __attribute__((unused))=0;
	uint32_t hexadecimal __attribute__((unused))=0;
	char *string __attribute__((unused))=0;
	char character __attribute__((unused))=0;
	while(format[i]!=0){	
		char ch = format[i++];
		...
		switch(state) {
			case 0:
				...
				break;
			case 1:
				//char *tmp = (char *)"In case 1\n";
	//			syscall(SYS_WRITE, STD_OUT, (uint32_t)((char *)"In case 1\n"), sizeof("In case 1\n"), 0, 0);
				if (ch == 'd') {
